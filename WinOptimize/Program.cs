...
namespace WinOptimize
{
    static class Program
    {
        ...
        const string _jsonAssembly = @"WinOptimize.Newtonsoft.Json.dll";

        internal static MainForm _MainForm;
        internal static SplashForm _SplashForm;

        static string _adminMissingMessage = "WinOptimize needs to be run as administrator!\nApp will now close...";
        static string _unsupportedMessage = "WinOptimize works with Windows 7 and higher!\nApp will now close...";
        ...
        static string _argInvalidMsg = "Invalid argument! Example: WinOptimize.exe /config=win10.json";
        static string _alreadyRunningMsg = "WinOptimize is already running in the background!";

        const string MUTEX_GUID = @"{EMRE37DESTAN-0EFC7B8A-D1FC-467F-B4B1-0117C643FE19-WINOPTIMIZE}";
        ...
        static void Main(string[] switches)
        {
            ...
            EmbeddedAssembly.Load(_jsonAssembly, _jsonAssembly.Replace("WinOptimize.", string.Empty));
            ...
            if (!_notRunning)
            {
                MessageBox.Show(_alreadyRunningMsg, "WinOptimize", MessageBoxButtons.OK, MessageBoxIcon.Information);
                Environment.Exit(0);
                return;
            }
            ...
            if (switches.Length == 1)
            {
                ...
                if (arg == "/version")
                {
                    if (!EXPERIMENTAL_BUILD) MessageBox.Show($"WinOptimize: {GetCurrentVersionTostring()}\n\nCoded by: Emre37Destan © ∞\n\nhttps://github.com/Emre37destan/WinOptimize", "WinOptimize", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    else MessageBox.Show("WinOptimize: EXPERIMENTAL BUILD. PLEASE DELETE AFTER TESTING.\n\nCoded by: Emre37Destan © ∞\n\nhttps://github.com/Emre37destan/WinOptimize", "WinOptimize", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                if (arg == "/disabledefender")
                {
                    ...
                    MessageBox.Show("Windows Defender has been completely disabled successfully.", "WinOptimize", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    Environment.Exit(0);
                    return;
                }
                ...
                if (arg.StartsWith("/config="))
                {
                    ...
                    {
                        MessageBox.Show(_confNotFoundMsg, "WinOptimize", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        Environment.Exit(0);
                        return;
                    }
                    ...
                    {
                        MessageBox.Show(_confInvalidFormatMsg, "WinOptimize", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        Environment.Exit(0);
                        return;
                    }
                    if (!SilentOps.ProcessWindowsVersionCompatibility())
                    {
                        MessageBox.Show(_confInvalidVersionMsg, "WinOptimize", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        Environment.Exit(0);
                        return;
                    }
                    SILENT_MODE = true;
        ...
        private static void LoadSettings()
        {
            ...
            try
            {
                ...
                //if (!Options.CurrentOptions.DisableWinOptimizeTelemetry)
                //{
        ...
        {
            // set RunOnce instruction
            Microsoft.Win32.Registry.SetValue(@"HKEY_LOCAL_MACHINE\Software\Microsoft\Windows\CurrentVersion\RunOnce", "*WinOptimizeEnableDefender", Assembly.GetExecutingAssembly().Location + " /silentenabledefender", Microsoft.Win32.RegistryValueKind.String);
            RestartInSafeMode();
        }

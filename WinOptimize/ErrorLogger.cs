...
namespace WinOptimize
{
    ...
    {
        internal static string ErrorLogFile = Path.Combine(CoreHelper.CoreFolder, "WinOptimize.log");
        ...
        internal static void InitializeSilentReport()
        {
            ...
            _silentReportLog.AppendLine(string.Format("WinOptimize {0} - .NET Framework {1} - Experimental build: {2}", Program.GetCurrentVersionTostring(), Utilities.GetNETFramework(), Program.EXPERIMENTAL_BUILD));
        ...
        internal static void GenerateSilentReport()
        {
            try
            {
                File.WriteAllText($"WinOptimize.SilentReport.{DateTime.Now.ToString("yyyyMMddTHHmm")}.log", _silentReportLog.ToString());
            }
            catch { }
        }
        ...
        internal static void LogError(string functionName, string errorMessage, string errorStackTrace)
        {
            ...
            try
            {
                if (!File.Exists(ErrorLogFile) || (File.Exists(ErrorLogFile) && File.ReadAllText(ErrorLogFile).Trim() == string.Empty))
                {
                    ...
                    File.AppendAllText(ErrorLogFile, string.Format("WinOptimize {0} - .NET Framework {1} - Experimental build: {2}", Program.GetCurrentVersionTostring(), Utilities.GetNETFramework(), Program.EXPERIMENTAL_BUILD));
                    File.AppendAllText(ErrorLogFile, Environment.NewLine);
                    File.AppendAllText(ErrorLogFile, Environment.NewLine);
                    File.AppendAllText(ErrorLogFile, Environment.NewLine);
                }
